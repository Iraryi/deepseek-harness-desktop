...
The Node half scans enabled Loader entries for web `dsh.client` packages, resolves each `exports["./client"]`, hashes the built bundle into the boot graph, and serves it with its source map under `/plugins`. Before the first and every later index response, it waits for Loader settlement, reconciles the live Loader tree, and retries package-resolution misses instead of caching them. A browser may connect as soon as the HTTP socket opens, but it cannot receive a partial startup graph while slow first-use profile initialization is still mounting entries; a moved installation can likewise heal its profile module fallback without leaving the running process on an empty boot graph. Source launch maps host imports to TypeScript source but still consumes this built client export; missing files share one build instruction followed by a package/path list, while unrelated filesystem errors remain separate failures.
