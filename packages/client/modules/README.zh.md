...
Node 侧会扫描已启用的 Loader 配置项以发现 web `dsh.client` 包，解析每个 `exports["./client"]`，把构建后的 bundle 哈希写入启动图，并通过 `/plugins` 提供该文件及其 sourcemap。在首次及之后每次返回 index 前，它都会等待 Loader 完成结算、重新核对实时 Loader 树，并重试而非缓存包解析缺失。浏览器可以在 HTTP socket 刚开放时连接，但在较慢的首次 profile 初始化仍在挂载配置项时，无法收到不完整的启动图；安装目录迁移时修复 profile 模块回退后，当前进程也不会继续提供空启动图。源码启动会把宿主侧导入映射到 TypeScript 源码，但仍消费这一构建后的客户端导出；缺失文件共享一条构建说明，随后以包／路径列表列出各项，而无关的文件系统错误仍是独立故障。
